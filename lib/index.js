...
internals.loadDocClient = function (driver) {
  if(driver) {
    internals.docClient = new DocClient({ service: driver, convertEmptyValues: true });
  } else {
    internals.docClient = internals.docClient || new DocClient({service : internals.dynamoDriver(), convertEmptyValues: true });
  }
...
internals.compileModel = function (name, schema) {
  ...
  Model.config = function(config) {
    ...
    if (config.docClient) {
      ...
      table.docClient = new DocClient({ service : config.dynamodb, convertEmptyValues: true });
    }
